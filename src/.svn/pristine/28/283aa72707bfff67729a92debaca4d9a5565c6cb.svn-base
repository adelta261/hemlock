...
public class Functions
{
	...
	public static void moveLocation(String newLoc) {		
		for(int i = 0; i<Game.world.getLocationList().toArray().length; i++) {
			if (newLoc.equals(Game.world.getLocationList().get(i).getLocationName())) {
				Game.world.setLocation(Game.world.getLocationList().get(i));
				TextDemo.print("You roam around the halls for greener pastures and arrive at the " + Game.world.getLocation().getLocationName());
				Functions.manipulateWindowTransition(2);
				Functions.populateScene();
				return;
			} 
		}
		TextDemo.print("That location does not exist.");
	}
